...
export const NavBody = ({ children, className, visible }) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-[1800px] flex-row items-center justify-between self-start rounded-full bg-transparent px-4 py-2 lg:flex",
        visible && "bg-white/80",
        className
      )}
    >
    ...
    >
      {items.map((item, idx) => (
        <Link
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-black font-josefin-sans uppercase hover:text-primary"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
